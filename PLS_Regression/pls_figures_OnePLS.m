...
text(NumRows+0.02,...
    ...
    'HorizontalAlignment','left')
title('Scores on the first PLS components');
xlabel('ObjNum');
ylabel('PLS-1');
...
hold on
plot(NumVarY,...
    scaledata(RESULTS.Y_Loadings(:,1),-1,1), 'v',...
    'MarkerFaceColor', 'black', ...
    'MarkerEdgeColor', 'black')
text(NumVarY,...
    scaledata(RESULTS.Y_Loadings(:,1),-1,1)+0.02, ...
    Y_TABLE.Properties.VariableNames,...
    'HorizontalAlignment','left')
title('BIPLOT of PLS-1 - Scaled in [-1,1]');
xlabel('PLS-1');
ylabel('PLS-2');
